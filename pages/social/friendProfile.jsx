...
import { studentId2 } from '../../constants/mockData';
...
export default function FriendProfile() {
  ...
  const { firstName, lastName, friendStudentId } = router.query;
  const friend = useSelector((state) => state.student).friend;
  const [friendState, setFriendState] = useState(user.student.friends.findIndex(x => x === friend._id) !== -1 ? 'friends' : user.student.outgoingFriendRequests.findIndex(x => x === friend._id) !== -1 ? 'requested' : 'request');
  ...
  const requestFriend = () => {
    ...
    const existingFriendRequest = user.student.outgoingFriendRequests.findIndex(x => x === friend._id) !== -1;
    if (!existingFriendRequest) {
      ...
      dispatch(updateStudent(user.student._id, { ...user.student, outgoingFriendRequests: outgoingFriendRequests }));
      dispatch(updateStudent(friend._id, { ...friend, incomingFriendRequests: incomingFriendRequests }));
      ...
      {friend && friend.user ?
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <FriendRequestButton state={friendState} onClickFunction={requestFriend}></FriendRequestButton>
